...
export default function CDialogCargarPonentes(props) {
  const { vAltoNav, setvSalasCargadas, vAccion,disabled } = props;
        ...
        const obj = csvHeader.reduce((object, header, index) => {
          switch (header.replace(" ", "").replace("\r", "").toLowerCase()) {
            ...
            case "celular":
              ...
              break;
            case "estatus":
              object[header.replace(" ", "").replace("\r", "").toLowerCase()] ="Inactiva"
              break;
            default:
              object[header.replace(" ", "").replace("\r", "").toLowerCase()] =
  ...
  return (
    <>
      <Mui.Button disabled={disabled} sx={{w:"150px"}} variant="contained" onClick={handleClickOpen}>
        {Variables.v_TEXTOS.cargar_ponentes}
      </Mui.Button>
      ...
      >
        <Mui.Stack
          ...
          spacing={2}
        >
          <Mui.DialogTitle>{Variables.v_TEXTOS.cargar_ponentes}</Mui.DialogTitle>
        </Mui.Stack>
        <Mui.Stack spacing={2}>
        <Mui.Stack
            direction="row"
